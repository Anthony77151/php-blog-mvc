  ...
  <div class="row white z-depth-3">
    <div class="col s12">
      <ul class="tabs tabs-fixed-width">
        <li class="tab"><a class="purple-text" href="#notSignaled"><strong>Non signalés</strong></a></li>
        <li class="tab"><a class="purple-text <?php echo ($this->aNbrSignals[0] > 0)?"active" : ""; ?>" href="#signaled"><strong>Signalés</strong></a></li>
      </ul>
    </div>
      		...
      		<?php if(!empty($this->oComments)): ?>
      			<?php foreach($this->oComments as $comment): ?>
      				<tr id="commentaire_<?= $comment->id ?>">
      					<td><a class="purple-text" href="blog_post_<?=$comment->post_id?>.html"><strong><?= $comment->title ?></strong></a></td>
      					<td><?= substr($comment->comment,0,100); ?></td>
      					<td>
      						<a id="<?= $comment->id ?>" class="btn-floating btn-small waves-effect waves-light deep-purple lighten-3 see_comment"><i class="material-icons">done</i></a>
      						<a id="<?= $comment->id ?>" class="btn-floating btn-small waves-effect waves-light deep-purple darken-3 delete_comment"><i class="material-icons">delete</i></a>
      						<a href="#comment_<?= $comment->id ?>" class="btn-floating btn-small waves-effect waves-light purple modal-trigger"><i class="material-icons">more_vert</i></a>
