...
<div class="container">
  <h2 class="white-text">Tableau de bord</h2>
  <div class="row">
    <?php for($i=0;$i<$this->length;$i++): ?>
			<div class="col l3 m3 s12">
				<div class="card">
					<div class="card-content <?= $this->aColors[$i] ?> black-text">
						<span class="card-title"><?= $this->aTableName[$i] ?></span>
						<h4 class="black-text"><?= $this->aInTable[$i][0] ?></h4>
					</div>
				</div>
			</div>
    <?php endfor ?>
  ...
  <div class="row white z-depth-3">
    <div class="col s12">
      <ul class="tabs tabs-fixed-width">
        <li class="tab"><a class="black-text" href="#notSignaled"><strong class="black-text">Non signalés</strong></a></li>
        <li class="tab"><a class="black-text <?php echo ($this->aNbrSignals[0] > 0)?"active" : ""; ?>" href="#signaled"><strong>Signalés</strong></a></li>
      </ul>
    </div>
      		...
      		<?php if(!empty($this->oComments)): ?>
      			<?php foreach($this->oComments as $comment): ?>
      				<tr id="commentaire_<?= $comment->id ?>">
      					<td><a class="black-text" href="blog_post_<?=$comment->post_id?>.html"><strong><?= $comment->title ?></strong></a></td>
      					<td><?= substr($comment->comment,0,100); ?></td>
      					<td>
      						<a id="<?= $comment->id ?>" class="btn-floating btn-small waves-effect waves-light grey lighten-1 see_comment"><i class="material-icons">done</i></a>
      						<a id="<?= $comment->id ?>" class="btn-floating btn-small waves-effect waves-light grey darken-4 delete_comment"><i class="material-icons">delete</i></a>
      						<a href="#comment_<?= $comment->id ?>" class="btn-floating btn-small waves-effect waves-light grey darken-2 modal-trigger"><i class="material-icons">more_vert</i></a>
      		...
      		<?php if(!empty($this->oSignaledComments)): ?>
      			<?php foreach($this->oSignaledComments as $signaledComment): ?>
      				<tr id="commentaire_<?= $signaledComment->id ?>">
      					<td><a href="blog_post_<?=$signaledComment->post_id?>.html"><strong class="black-text"><?= $signaledComment->title ?></strong></a></td>
      					<td><?= substr($signaledComment->comment,0,100); ?></td>
                <td><?= $signaledComment->signals ?></td>
      					<td>
      						<a id="<?= $signaledComment->id ?>" class="btn-floating btn-small waves-effect waves-light grey lighten-1 see_comment"><i class="material-icons">done</i></a>
      						<a id="<?= $signaledComment->id ?>" class="btn-floating btn-small waves-effect waves-light grey darken-4 delete_comment"><i class="material-icons">delete</i></a>
      						<a href="#comment_<?= $signaledComment->id ?>" class="btn-floating btn-small waves-effect waves-light grey darken-2 modal-trigger"><i class="material-icons">more_vert</i></a>
